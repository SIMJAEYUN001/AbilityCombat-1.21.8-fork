...
| **EarthAwakening** | `EarthAwakening.java` | 대시 경로 균열 | **신규** | 8칸 대시 후 0.7초 지연 균열 |
...
| **TapDancer** | `TapDancer.java` | 공격 템포 스택 | **신규** | 최대 8스택 이동속도 증가 |
...
| **대지의 각성** | 웅크리기+철괴 우클릭으로 8칸 대시, 0.7초 뒤 경로 균열이 반경 2칸 피해 3과 30% 슬로우 2초 부여 |
...
| **탭 댄서** | 근접 공격마다 템포 1스택, 스택당 이동 속도 +0.012, 최대 8스택, 4초 미전투마다 1스택 감소 |
