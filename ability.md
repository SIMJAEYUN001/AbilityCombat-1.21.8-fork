...
| **SwordDance** | `SwordDance.java` | 순차 텔레포트 타격 | **신규** | 5칸 내 최대 4명, 1.2초 무적 |
...
| **OrbitalLaser** | `OrbitalLaser.java` | 지점 예고 레이저 | **신규** | 20칸 지정, 1초 예고 후 피해/실명 |
...
| 능력 | 변경 내용 |
|------|----------|
| **검무** | 철괴 우클릭으로 5칸 내 적 최대 4명에게 0.2초 간격으로 이동 타격, 타격당 피해 4, 시전 중 1.2초 무적 |
| **대지의 각성** | 스프린트 게이지 대시 후 이동 경로 균열이 0.15초 간격으로 순차 폭발, 반경 2칸 피해 3과 30% 슬로우 2초 부여 |
| **연쇄 번개** | 근접 공격 적중 시 주변 5칸 내 최대 3명에게 원본 최종 피해 35% 전이, 대상별 쿨타임 없음 |
| **궤도형 레이저** | 철괴 우클릭으로 최대 20칸 지점에 1초 경고 원 표시 후 반경 2.5칸 피해 10과 실명 2초 부여 |
...
| 항목 | 변경 내용 |
|------|----------|
| **망령화** | 스택 표시를 보스바로 변경, 피해 시 +6스택, 필드 이탈 시 현재 스택 80% 손실, 비전투 감소 시작 7초, 해제 후 재진입 대기 15초 |
| **악마의 부츠** | 실제 불 블록 대신 화염 파티클 자취를 남기고, 닿은 적에게 매초 무제한 화상 스택을 부여. 화상은 스택당 초당 0.5 피해, 물/소화 시 초기화 |
