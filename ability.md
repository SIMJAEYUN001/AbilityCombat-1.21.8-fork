...
신규 능력은 대부분 철괴 우클릭 액티브 1개와 쿨타임 22~56초를 사용하며, `유리 대포`만 패시브 피해 증폭으로 동작합니다. 대상 판정은 source-aware `LocationUtil.isValidTarget(source, target)` 필터를 사용합니다. 직접 피해는 Bukkit 피해 이벤트를 발생시켜 `GameManager#onEntityDamageByEntity`의 팀킬 차단과 기존 전투 보정을 통과합니다. 빙결 계열은 이미 빙결된 대상을 맞히면 시전자를 1 회복합니다.
