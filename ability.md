...
| **WraithForm** | `WraithForm.java` | 망령화 스택, 말뚝 필드, 속박 | **신규** | 50스택 이상 망령 상태 전환 및 필드 이탈 패널티 |
...
| 방어 / 반발 / 팀 지원 / 무작위 / 피해 증폭 | 생존, 팀 보조, 변수 창출, 패시브 증폭 |

신규 능력은 대부분 철괴 우클릭 액티브 1개와 쿨타임 20~44초를 사용하며, `유리 대포`만 패시브 피해 증폭으로 동작합니다. 대상 판정은 source-aware `LocationUtil.isValidTarget(source, target)` 필터를 사용합니다. 직접 피해는 Bukkit 피해 이벤트를 발생시켜 `GameManager#onEntityDamageByEntity`의 팀킬 차단과 기존 전투 보정을 통과합니다. 빙결 계열은 이미 빙결된 대상을 맞히면 시전자를 1 회복합니다.

능력 선택/정보 설명에는 각 능력의 실제 적용 수치를 기존 능력처럼 설명 문장 안에 섞어 노출합니다.

| 설명 항목 | 기재 내용 |
|---|---|
| 피해/범위 | 단일 피해, 광역 반경, 사거리, 넉백/견인 강도 |
| 버프 | 신속/저항/흡수/재생의 레벨과 지속시간, 회복량 |
| 디버프 | 출혈 피해 주기와 총 지속시간, 감염 받는 피해 증가량, 실명/멀미/위더/나약함/발광/구속 지속시간 |
| 군중제어 | 기절/속박/무장해제/빙결의 정확한 지속시간 |
| 특수 조건 | 처형 체력 기준과 피해량, 도박 4분기 효과, 순간이동 거리, 빙결 재타격 회복 |

쿨타임은 기존 이름별 요청값을 상한으로 두고, 실제 밸류에 맞춰 스타일별 기준값을 적용합니다. 단일/도박/순간이동 계열은 20~24초대로 낮추고, 팀 지원/방어는 30초 안팎, 중력장/빙결 같은 광역 CC만 36~44초 범위에 남깁니다.
...
| 항목 | 변경 내용 |
|------|----------|
| **망령화 (WraithForm)** | 피해 시 망령화 스택을 얻고, 50스택 이상에서 말뚝/18칸 필드와 피해 증가/사거리 증가/흡혈을 활성화 |
| **말뚝 필드** | 정원사 나무처럼 능력 전용 ArmorStand로 말뚝을 생성하되 파괴 로직은 제외, 필드 경계 이펙트와 이탈 시 스택 감소만 처리 |
...
| **추가 효과** | 기습 순간이동, 중력장, 출혈, 감염, 위치교환, 표식, 처형, 팀 지원, 무작위 효과, 패시브 피해 증폭 구현 |

### 2026-06-04 신규 100개 능력 설명/밸런스 보정

| 항목 | 변경 내용 |
|------|----------|
| **설명 수치화** | 신규 100개 능력의 GUI/정보 설명에 피해, 사거리, 반경, 포션 레벨/시간, 디버프 수치, CC 지속시간을 별도 수치 라벨이 아니라 설명 문장 안에 삽입 |
| **쿨타임 재조정** | 22~56초 분포를 20~44초 중심으로 낮추고, 스타일별 밸류에 따라 단일/도박/순간이동 계열을 더 짧게 조정 |
| **감염 디버프 수정** | 감염이 약화 설명과 맞게 받는 피해 +25% 디버프로 동작하도록 수정 |
| **도박 계열 보정** | 무작위 능력의 폭발/견인 분기가 실제로 작동하도록 반경 3.6칸, 조준거리 10칸, 회복 2.0을 부여 |
| **제우스 조정** | 돌진 2.5초, 속도 1.5로 하향, 재사용 순간이동은 돌진 거리 안에 구조물과 안전 발판이 있을 때만 허용 |
