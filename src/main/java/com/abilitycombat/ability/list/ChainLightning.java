...
@AbilityManifest(name = "연쇄 번개 (ChainLightning)", species = AbilityManifest.Species.SPECIAL, explain = {
        "§e§l[패시브 - 연쇄]",
        "§7근접 공격이 적중하면 피격자 주변 §f3칸§7 내 추가 적을 찾습니다.",
        "§7최대 §f3명§7에게 원본 최종 피해의 §e35%§7 전이 피해를 줍니다.",
        "§7전이 대상별 쿨타임은 §f2초§7이며, 실제 번개 없이 파티클과 효과음만 출력합니다."
}, summarize = {
        "§7패시브§f: 근접 적중 시 주변 3명에게 35% 전이 피해",
        "§7전이 대상§f: 대상별 2초 쿨타임"
})
public class ChainLightning extends AbilityBase {

    private static final double CHAIN_RADIUS = 3.0;
    private static final int MAX_CHAIN_TARGETS = 3;
    private static final double CHAIN_DAMAGE_RATIO = 0.35;
    private static final long CHAIN_COOLDOWN_MILLIS = 2_000L;
    private static final long CACHE_CLEANUP_MILLIS = 10_000L;
    private static final Particle.DustOptions LIGHTNING_DUST =
            new Particle.DustOptions(Color.fromRGB(255, 230, 90), 1.0f);

    private final Map<UUID, Long> lastChainTime = new HashMap<>();
    ...
    public void handleBridgeEvent(Event event) {
        ...
        LocationUtil.getNearbyLivingEntities(victim.getLocation(), CHAIN_RADIUS, player,
                target -> !target.equals(victim) && !(target instanceof ArmorStand) && canChain(target, now))
                .stream()
                .sorted(Comparator.comparingDouble(target -> target.getLocation().distanceSquared(victim.getLocation())))
                .limit(MAX_CHAIN_TARGETS)
                .forEach(target -> chainTo(player, origin, target, chainDamage, now));
    }

    private boolean canChain(LivingEntity target, long now) {
        return now - lastChainTime.getOrDefault(target.getUniqueId(), 0L) >= CHAIN_COOLDOWN_MILLIS;
    }

    private void chainTo(Player player, Location origin, LivingEntity target, double damage, long now) {
        lastChainTime.put(target.getUniqueId(), now);
    ...
    private void cleanupCache(long now) {
        lastChainTime.entrySet().removeIf(entry -> now - entry.getValue() > CACHE_CLEANUP_MILLIS);
    }
}
